  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
      <div className="bg-primary text-white text-xs py-2 px-4 hidden md:block">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <span className="flex items-center gap-1"><Mail size={12} /> info@dentalclinic.com</span>
          </div>
          <div className="flex items-center gap-4">
      ...
      <nav className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        <div className="text-2xl font-bold text-primary">DentalClinic</div>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 p-4 space-y-4">
          ...
          <button className="w-full bg-primary text-white px-5 py-3 rounded-full text-sm font-semibold hover:bg-blue-700 transition">Book Appointment</button>
        </div>
      )}
    </header>
  );
